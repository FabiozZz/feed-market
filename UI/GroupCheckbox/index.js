...
const GroupCheckbox = ({children, onClick, ...other}) => {
    ...
    useEffect(() => {
        ...
        let checkbox = [];
        setContent(content?.map((item, index) => {
    ...
    const clearCheckBox = (array, groupChecked, isClear) => {
        ...
        checkboxes.forEach((item, i) => {
            ...
            return i !== 0 ? changeContent[item.index] = cloneElement(element, {...element.props, checked: check, groupChecked: groupChecked}) :
                !isClear ? element : changeContent[item.index] = cloneElement(element, {...element.props, checked: false});
        })
